...
def create_app(test_config=None): # factory function
    app = Flask(__name__, instance_relative_config=True) # config files are relative to instance folder
    ...
    if test_config is None:
        ...
        app.config.from_pyfile('config.py', silent=True) # can be used to set the secret key
    else:
    ...
    try:
        os.makedirs(app.instance_path) # creates the instance folder if does not exists, will be storing the database file
    except OSError:
        pass
