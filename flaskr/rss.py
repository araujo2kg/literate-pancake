...
def rss():
    ...
    for post in posts:
        ...
        fe.author(
            {
                "name": post["username"],
                "email": f"{post['username']}@example.com",
            }
        )
        fe.published(post["created"].replace(tzinfo=pytz.UTC))
        if post["imagename"]:
            fe.enclosure(
                url=f"http://127.0.0.1:5000/image/{post['imagename']}",
                length="500000",
                type=f"image/{os.path.splitext(post['imagename'])[1][1:]}",
            )
        else:
            fe.enclosure(
                url="http://127.0.0.1:5000/image/lena.bmp",
                length="500000",
                type="image/bmp",
            )
